...
if [[ $? -ne 0 ]] ; then
 ...
 if [[ $? -eq 0 ]] ; then
  ...
  docker run -ti $MORE_ARGS --hostname mail.wikitolearn.org --name ${INSTANCE_NAME}-mailsrv -d debian
 fi
fi
...
if [[ $? -ne 0 ]] ; then
 ...
 if [[ $? -eq 0 ]] ; then
  ...
  docker run -ti $MORE_ARGS --hostname ocg.wikitolearn.org --name ${INSTANCE_NAME}-ocg -d debian
 fi
fi
