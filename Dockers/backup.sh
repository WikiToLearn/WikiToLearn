...
BACKUP_DIR=$W2L_BACKUP_PATH"/"$(date +'%Y_%m_%d__%H_%M_%S')
...
for db in $(docker exec -ti ${W2L_INSTANCE_NAME}-mysql mysql -e "SHOW DATABASES" | grep wikitolearn | awk '{ print $2 }') ; do
 ...
 docker exec -ti ${W2L_INSTANCE_NAME}-mysql mysqldump --skip-add-drop-table --skip-comments -d $db > $BACKUP_FILE_STRUCT
 sed -i 's/CREATE TABLE/CREATE TABLE IF NOT EXISTS/g' $BACKUP_FILE_STRUCT

 docker exec -ti ${W2L_INSTANCE_NAME}-mysql mysqldump --no-create-info $db > $BACKUP_FILE_DATA
done
