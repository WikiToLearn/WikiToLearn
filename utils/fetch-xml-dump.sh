...
curl -d "&templates&curonly&action=submit&pages=$(cat dump-list | hexdump -v -e '/1 "%02x"' | sed 's/\(..\)/%\1/g' )" http://it.wikitolearn.org/index.php?title=Special:Export -o "../developer-dump.xml"
